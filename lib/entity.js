...
module.exports = class Entity extends Request {
  ...
  constructor(options) {
    ...
    this.options.methods.OPTIONS = options.resourceInfo || false;
  ...
  get(identifier, format = 'json') {
    return this.issueRequest(methods.get, `${this.options.methods.GET.replace(this.options.methods.GET.match(/\{.*?\}/), identifier)}?_format=${format}`, '');
  }
  ...
  patch(identifier, body = {}, format = 'application/json') {
    return this.getXCSRFToken()
      .then((csrfToken) => this.issueRequest(methods.patch, `${this.options.methods.PATCH.replace(this.options.methods.PATCH.match(/\{.*?\}/), identifier)}`, csrfToken, {'Content-Type': format}, body));
  }
  ...
  post(body, format = 'application/json') {
    return this.getXCSRFToken()
      .then((csrfToken) => this.issueRequest(methods.post, this.options.methods.POST, csrfToken, {'Content-Type': format}, body));
  }
  ...
  delete(identifier) {
    return this.getXCSRFToken()
      .then((csrfToken) => this.issueRequest(methods.delete, `${this.options.methods.DELETE.replace(this.options.methods.DELETE.match(/\{.*?\}/), identifier)}`, csrfToken));
  }
  ...
  */
  getFieldData(format = 'json'){
    if (!this.options.methods.OPTIONS) {
      return Promise.reject(`No available OPTIONS path for ${this.options.bundle}.`);
    }
    return this.issueRequest(methods.get, `${this.options.methods.OPTIONS.replace(this.options.methods.OPTIONS.match(/\{.*?\}/), this.options.bundle)}?_format=${format}`)
      .then(bundleMetadata => {
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(() => {
        ...
        if (nonMatchedFields.length){
          return reject(new Error(`The ${nonMatchedFields.length > 1 ? 'fields' : 'field'}, ${nonMatchedFields.join(', ')}, ${nonMatchedFields.length > 1 ? 'are' : 'is'} not included within the bundle, ${this.options.bundle}.`));
        }
        const bodyContent = {type: this.options.bundle};
