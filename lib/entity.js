...
module.exports = class Entity extends Request {
  ...
  get(token, format = 'json') {
    return this.issueRequest(methods.get, `${this.paths.GET.replace(this.paths.GET.match(/\{.*?\}/), token)}?_format=${format}`, '');
  }
   ...
   */
  patch(token, body = {}, format = 'json') {
    return this.getXCSRFToken()
      .then((csrfToken) => this.issueRequest(methods.patch, `${this.paths.PATCH.replace(this.paths.GET.match(/\{.*?\}/), token)}?_format=${format}`, csrfToken, {}, body));
  }
  ...
  delete(token) {
    return this.getXCSRFToken()
      .then((csrfToken) => this.issueRequest(methods.delete, `${this.paths.DELETE.replace(this.paths.GET.match(/\{.*?\}/), token)}`, csrfToken));
  }
  ...
  setField(entity, field, value) {
    return new Promise((resolve, reject) => {
      ...
      .then(() => {
        ...
        return this.patch(entity, bodyContent)
