...
module.exports = class Entity extends Request {
  ...
  constructor(options) {
    ...
    this.options.methods.OPTIONS = options.more || false;
  ...
  patch(identifier, body = {}, format = 'application/json') {
    return this.getXCSRFToken()
      .then((csrfToken) => this.issueRequest(methods.patch, `${this.options.methods.PATCH.replace(this.options.methods.GET.match(/\{.*?\}/), identifier)}`, csrfToken, {'Content-Type': format}, body));
  }
