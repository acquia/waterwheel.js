...
module.exports = class Request extends Base {
   ...
   *   The time used to check to see if the token has expired.
   * @returns {Promise}
   *  A Promise that when fulfilled returns a response from the request.
   */
  issueRequest(method, url, XCSRFToken, additionalHeaders, body, baseOverride, currentTime = new Date().getTime()) {
    return (
      this.oauth.tokenInformation.access_token && (
        this.oauth.hasOwnProperty('tokenExpireTime') &&
        this.oauth.tokenExpireTime > currentTime
      ) ? Promise.resolve() : (
        (this.oauth.hasOwnProperty('tokenExpireTime')
        && this.oauth.tokenExpireTime < currentTime )
      ) ? this.oauth.refreshToken() : this.oauth.fetchToken()
    )
    .then(() => {
      const options = {
        method: method,
