 ...
 */
  get(resource, params, id = false, langcode = false) {
    const url = `${langcode ? `/${langcode}` : ''}/${this.jsonapiPrefix}/${resource}${id ? `/${id}` : ''}${Object.keys(params).length ? `?${qs.stringify(params, {indices: false})}` : ''}`;
