...
test('Existing OAuth Token', t => {
  ...
  request.oauth.tokenExpireTime = time.getTime();
  request.oauth.tokenInformation.access_token = '9876';

  time.setSeconds(+time.getSeconds() + (-100));

  return request.issueRequest(methods.get, '/entity/1', '12345', false, false, false, time.getTime())
    .then(res =>
      t.deepEqual(res, {
        method: 'GET',
        timeout: 500,
...
test('Existing Expired OAuth Token', t => {
  rs.subvert('axios', options => Promise.resolve({data: options}));
  const Request = rs.require('../lib/helpers/request');
  const request = new Request(t.context.options, new t.context.oauth(t.context.baseURL, t.context.oauthOptions));
  ...
  return request.issueRequest(methods.get, '/entity/1', '12345', false, false, false, time.getTime())
    .then(res => {
      t.is(request.oauth.tokenInformation.refresh_token, '000');
      t.deepEqual(res, {
        method: 'GET',
        timeout: 500,
        url: 'http://drupal.localhost/entity/1',
